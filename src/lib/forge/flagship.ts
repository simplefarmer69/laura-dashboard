...
export const LAB_REGISTRY: FlagshipSpec = {
  ...
  announceShape:
    "three plain sentences in LAURA's first person. The Ownership Market (where a contract's owner sells it to any wallet) was ANNOUNCED SEPARATELY and the linked contract here is the registry, so do not re-describe the market's mechanics or call The Lab a market. FIRST sentence: what this contract is and what changes hands: The Lab, the storefront over the Ownership Market on Robinhood Chain, an on-chain registry where the seller of a listing publishes the name, description, image, github, socials and audit links buyers see. SECOND: what a wallet does there: sellers write it with setMetadata from the Lab page, buyers and any frontend read it for free, so anyone can host the same storefront. THIRD: the Lab link and the explorer link (the guide link only if it fits). Name the Ownership Market in a few words as the product this extends; never open with \"the lab is open\" alone.",
  fallbackPost:
