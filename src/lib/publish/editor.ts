import { z } from "zod";
import { TWEET_MAX, X_STYLE_GUIDE, acceptAuditEdit, closesWithAphorism, sanitizeXPost } from "@/lib/publish/x-style";
...
export function readabilityProblems(text: string): string[] {
  ...
  if (closesWithAphorism(t)) problems.push("closes on an aphorism with no number, date or source; delete the last sentence and end on the fact");
...
export function editorPrompt(input: { drafts: Draft[]; recent: XPostLogEntry[]; today: string }): string {
  ...
  return [
    ...
    `THE READER TEST, apply it literally to every post: (1) After the first sentence, can the reader say in plain words what this post is about? (2) Does every number carry a unit and a referent a human uses (dollars, ETH, holders, launches, percent, a date), never a bare counter like "1089 to 1113" or a pipeline label like "Experiment 56"? (3) Is there any word that only the pipeline understands: "third call", "sale clock", "proxy", "lever", "lane", "resubmitted", "the spine", "this cycle", "vetoed"? (4) Does the reader walk away with one concrete thing about StonkBrokers, Robinhood Chain, a launch, a market or LAURA's own onchain moves that they did not know, or one clear view they can agree or disagree with? (5) Would a human read it and think a person who knows the chain wrote it, in complete thoughts? (6) Does it STOP AT THE LAST FACT, or does it bolt on a closing verdict that carries no number, date or source ("fees trail swaps", "depth first, volume second", "monday's open is the test", "the holder line does not care what the candle did")? The operator flagged this on 2026-09-16 as the account's most repeated tell: twelve of thirty-seven posts ended the same way. Failing (6) is always a "rewrite", never a "hold", and the rewrite is simply the post with that last sentence deleted.`,
