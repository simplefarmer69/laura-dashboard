...
export function xAuditPrompt(input: {
  ...
  return [
    ...
    /^smith$|\(smith\)$/.test(input.author.trim())
      ? `THIS IS A CONTRACT ANNOUNCEMENT from Anvil (smith): a newly verified contract with its address in the link. Earlier posts announcing OTHER contracts share its theme by design (the swarm ships contracts; each gets one announcement) and are not duplicates of it. Judge repetition on phrasing only: veto if it reuses an earlier post's opening, clause or sentence shape, pass if it says in its own words what THIS contract does and how a person uses it. The links are required and count for nothing.`
      : "",
