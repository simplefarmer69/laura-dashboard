  ...
  return [
    ...
    m.ecosystemVolume24hUsd == null
      ...
      : m.ecosystemVolumeVersion === 2
        ? `Ecosystem volume 24h (every fee-bearing tape, all-in): ${usd(m.ecosystemVolume24hUsd)} = $STONKBROKER DEX ${usd(m.tokenDexVolume24hUsd)} + Special Projects ${usd(m.specialProjectsVolume24hUsd ?? 0)} + ${m.launcherTokenCount ?? 0} launcher tokens ${usd(m.launcherTokensVolume24hUsd ?? 0)} + Smart LP vault share ${usd(m.smartLpAttributedVolume24hUsd ?? 0)} of ${usd(m.smartLpPoolsGrossVolume24hUsd ?? 0)} gross across ${m.smartLpPoolCount ?? 0} pools`
        : `Ecosystem volume 24h (excluding $STONKBROKER): ${usd(m.ecosystemVolume24hUsd)} = Special Projects + LAURA pairs ${usd(m.ecosystemTokensVolume24hUsd ?? 0)} + Smart LP vault share ${usd(m.smartLpAttributedVolume24hUsd ?? 0)}`,
