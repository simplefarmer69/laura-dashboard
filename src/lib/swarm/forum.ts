...
/** Titles that baked a relative age at pour time (the Vlad veto tab still
 * says "an hour ago" 17h later). Flag them so nobody copies the frozen age. */
const FROZEN_AGE_RE =
  /\b(\d+\s*(m|min|mins|minutes|h|hr|hrs|hours|d|day|days)\s+ago|an hour ago|a day ago|thirty minutes|sixty minutes|minutes old|hours old|days old)\b/i;

function displayTitle(title: string): string {
  return FROZEN_AGE_RE.test(title)
    ? `${title} [TITLE AGE FROZEN AT POUR — not current; read THE WIRE's UTC stamp]`
    : title;
}

const CLOCK_RULE =
  "CLOCK (operator 2026-09-12): THE WIRE stamps every tweet with a live relative age AND a UTC time. Relative ages in thread TITLES are frozen at pour (the tab \"Vlad asked an hour ago\" opened 2026-09-11 21:29 when the tweet was 60 minutes old; it is still open hours later). Never copy a title's relative age into a new post as if it were now. Cite the UTC stamp from THE WIRE. Never put \"an hour ago\" / \"30 minutes old\" in a new title — use the UTC time. A tab whose only hook is a stale relative age is done; last-call it.";
...
export function forumDigest(threads: ForumThread[]): string {
  ...
  return open
    ...
    .map((t) => {
      ...
      return `THREAD ${t.id} [${t.tag}] "${displayTitle(t.title)}", opened by ${t.createdBy}, ${t.posts.length} post(s)${hidden ? ` (${hidden} earlier not shown)` : ""}\n${posts || "    (no replies yet)"}`;
    })
    .join("\n\n");
}
...
/**
 * Everything in a turn prompt that is identical for every agent in a round,
 * built once when the round opens and sent as a cached block ahead of each
 * turn. The venue is snapshotted here rather than read live: it is by far
 * the largest part of the prompt, it only ever changes by appending this
 * round's posts, and a prefix cache needs it byte identical across the 28
 * turns. The live part, posts landed since the round opened, rides in the
 * turn prompt as a delta, so every agent still sees the whole board.
 */
export function forumSharedBlock(state: SwarmState, wire: string): string {
  ...
  return [
    ...
    `THE VENUE AT ROUND OPEN (ten liveliest tabs in detail)\n${forumDigest(state.forum ?? [])}`,
...
function forumPrompt(agent: Agent, state: SwarmState, roundId: string): string {
  return [
    ...
    `NEW THIS ROUND (posts landed since THE VENUE AT ROUND OPEN was taken; read with it for the live board)\n${roundDeltaDigest(state.forum ?? [], roundId, state.agents)}`,
...
function hostVenueIndex(threads: ForumThread[]): string {
  ...
  return open
    .map((t) => {
      ...
      return `- ${t.id} [${t.tag}] "${displayTitle(t.title)}" opened by ${t.createdBy}, ${t.posts.length} post(s), last activity ${idleH}h ago`;
    })
    .join("\n");
}
...
export async function runForumRound(): Promise<ForumRoundResult> {
  ...
  try {
    ...
    const wire = `${intel}\n${offProtocol}`;
    /* Built once here and cached for the whole round; see forumSharedBlock. */
    const shared = forumSharedBlock(state, wire);
    ...
    for (const agent of order) {
      try {
        const out = await generateStructured(resolved, {
          ...
          prompt: forumPrompt(agent, state, roundId),
          stable: [shared],
