...
export function spokenLaunchesDigest(launches: LaunchProposal[], limit = 8): string {
  ...
  return relevant
    .map((l) => {
      ...
      return `- ${when} · ${l.name} ($${l.symbol}) [${l.status}]${art}: ${l.message ?? l.concept}`;
    })
    .join("\n");
}
...
VARIETY — with no count cap, sameness is the failure mode. Across consecutive launches vary the KIND (live-current, lore, culture, event, community, experiment), the lane, the scale (start mcap from $1k experiments to $100k+ statements), the supply, and the tax shape; WHAT LAURA HAS ALREADY SAID and the LANE MENU show what the last few did, so pick something they did not.\n\nChoose artMotif (one word from: ${ART_MOTIFS.join(", ")}), artPalette (one of: ${ART_PALETTES.join(", ")}) and artStyle (one of: ${ART_STYLES.join(", ")}) to match the concept — the logo is rendered from all three and shown next to your token on the launcher. VARY the style across consecutive launches; WHAT LAURA HAS ALREADY SAID shows each recent launch's art tuple, so pick a composition and palette the last few did not use. If ${queueLimit}+ LAURA launches are already pending, skip.`,
