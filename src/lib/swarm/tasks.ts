...
function xPostBrief(ctx: CycleContext, agentId: AgentId): string {
  return [
    ...
    `THE ALPHA SHAPE (operator directive 2026-09-13: influence on X comes from being read, and the account this swarm studies, @aixbt_agent, earns 15-25k views a post with one shape; use it). Three sentences, lowercase is fine, no hashtags, no emoji, no question, no call to action: (1) CAUSE THEN EFFECT: "after <event>, <what changed for whom>" in plain words a stranger follows; (2) THE NUMBERS: two or three figures with units and windows that prove sentence one, at least one of them COMPARATIVE (a chain rank, a share of all crypto, a protocol's rank on the chain, a gap to the next name, a change vs the prior week); the MARKET lines in CHAIN ALPHA and LAURA's own feeds (launcher tape, holders, pairs, fee breakdown) are the sources; (3) THE IMPLICATION: what follows if it holds and the one condition that would break it. Robinhood Chain set against all of crypto is the edge nobody else posts: use it whenever the numbers carry it.`,
    `$LAURA IS HERS (operator directive 2026-09-13, say it plainly when relevant): LAURA herself designed and launched "LAURA Is Online" ($LAURA, launch 276, WETH lane, 0x04921d4c9Fc16fe86B995a54696104A128C51387) and every token on her launch list. This is good and she owns it in public: every strong launch of hers is attention for the Stonkbrokers ecosystem and curve fees that flow to $STONKBROKER holders, so she may cite her own launches as worked examples with their real numbers (holders, trades, fees, phase). She never buys or trades them, never talks price or returns, and never hides that she launched them.`,
...
export const treasurerSchema = z.object({
  ...
  actions: z
    .array(
      z.object({
        action: z.enum(["hold", "unwrap-weth", "buy-stonk", "lp-enter", "lp-exit", "collect-earnings", "eco-buy", "eco-sell", "ns-buy", "ns-sell", "bridge-arb"]),
        /** ETH for unwrap-weth (omit = all), eco-buy, ns-buy and bridge-arb; ignored elsewhere. */
        ...
        /** Nightshades faction for ns-buy / ns-sell: ghosts, watchers, knights or zombies. */
        faction: z.enum(["ghosts", "watchers", "knights", "zombies"]).nullable(),
        /** For eco-sell / ns-sell: share of the held balance to sell, 0-1. */
...
export interface TreasurerInputs {
  ...
  candidates: string;
  /** Nightshades game clock, per-faction numbers and LAURA's positions (deterministic). */
  nightshades: string;
  /** Arbitrum One balance, bridge ledger and top-up caps (deterministic). */
  bridge: string;
...
export function treasurerPrompt(ctx: CycleContext, input: TreasurerInputs): string {
  return [
    ...
    `NIGHTSHADES (the Meebco x Clutch Markets survival game on Robinhood Chain; deterministic reads this cycle)\n${input.nightshades}\nHow the game works, in the numbers that matter for a trade: four faction tokens (ghosts, watchers, knights, zombies), each in its own protocol-owned Uniswap v4 pool against WETH with a 1% fee. Once a day The Night strikes: Chainlink VRF picks the survivors (75% of Nights exactly one faction survives and three are struck, 25% two survive), 20-80% of every struck faction's pool liquidity is withdrawn and struck tokens are sold into their own pools (price down), and that ETH plus the boost pot is added to the survivors' pools (price up). Trading halts for the Night itself, then reopens at Sunrise with a 99% anti-snipe tax that decays to 0% over 60 minutes on buys AND sells. A struck faction is not eliminated and can be struck again. So: a position held through a Night is a wager with roughly a 31% chance per faction of surviving; holding all four guarantees exposure to the strike and to the survivors' gain; buying a struck faction after Sunrise is a bet on recovery flows (the tax split funds the next Night's boost); selling before a Night takes chips off the table. The special-project angle: every faction trade pays the 1% pool fee and 24h volume on these four pairs already runs in the tens of ETH, and their launch tax flowed through the StonkLauncher. Use ns-buy with faction and amountEth, ns-sell with faction and fraction. Always weigh the clock line first: nothing executes during a Night, and the Sunrise tax ceilings differ by side. READ THE CLOCK BEFORE ANYTHING ELSE. The NOW line at the top of the block gives the wall-clock time in UTC, and every Night, Sunrise and "clears at" time in this brief is UTC with a Z suffix, the same zone the game's contracts and API use. Work out where you are against that clock before you decide: how many minutes until the Night, whether a Sunrise window is open, how long is left in it. A window you miss because you read a schedule without checking the time is the one avoidable way to lose this game.

TRADE THE SUNRISE WINDOW (operator directive 2026-09-16). The hour after every Night is the only part of the day with a readable edge, and until now the swarm sat it out: a single 1% ceiling cleared with 36 seconds left in the hour, so every trade landed outside the window at a fair price. The ceilings are now 10% on a BUY and 1% on a SELL, and the SUNRISE line in the digest gives you the window's open and close, minutes remaining, and the clock time each ceiling clears. Three rules:
(1) SELL BEFORE THE NIGHT, NOT AFTER. Selling is free in the hours before a Night and the digest marks the 90 minutes ahead of it as the exit window. A position you would not want struck should leave then. Selling a faction after it has been struck pays twice, once in the lower price and once in the tax, which is why the sell ceiling stays at 1%.
(2) NEVER BUY IN THE FIRST HALF OF SUNRISE. The tax decays linearly from 99%, so it is still about 50% at the half hour. No discount in this game is worth 50%. Ignore the window until the digest says a ceiling is minutes away.
(3) BUY STRUCK FACTIONS LATE IN THE WINDOW. A struck pool has just had 20-80% of its liquidity pulled and its own tokens sold into it, so it is at its cheapest of the day while the survivors hold the boost pot. In the last ten minutes the tax is in single digits against a discount that is usually tens of percent. Name both numbers in your reason: the tax you will pay right now, and how far below its pre-Night price the pool is. If the tax still exceeds the discount, say so and wait; a window you correctly skipped is a good decision, and there is another one tomorrow.
A buy minutes before a Night is a coin flip you must name as one. FIRST TRADE (operator directive 2026-09-15): while the Nightshades sleeve is empty and the clock line says trading is open, prefer one small ns-buy this cycle over another eco-buy — get a seat in the game, then manage it. Eco and Nightshades are both valid; an empty Nightshades ledger with an open clock is the cue to start.`,
    `ARBITRUM ONE (the launcher's second chain, operator directive 2026-09-15; deterministic reads this cycle)\n${input.bridge}\nThe StonkBrokers launcher runs the same Smart Launch V2 pads on Arbitrum One (chain 42161); Mint and Ticker deploy there through the arbweth lane, paying gas from the wallet's Arbitrum balance. The operator bridged 1 ETH there as launch capital. Your job on this chain is narrow: keep the arbweth lane fundable. bridge-arb moves a small ETH top-up from Robinhood Chain to Arbitrum One through Relay (an intent bridge; the fill lands in seconds and the fee is about 0.1%). It only fires while the Arbitrum balance is under the top-up line, so listing it when the balance is healthy is a no-op the ledger will show as skipped.`,
...
export function treasurerMock(): TreasurerOut {
  return {
    ...
    actions: [{ action: "hold", amountEth: null, token: null, faction: null, fraction: null, reason: "Deterministic fallback (no LLM key): hold until a live model reads the sleeves." }],
  ...
  actions: z
    .array(
      z.object({
        ...
        /**
         * The brief: 2 to 4 sentences and 700 characters, executable by a
         * stranger without asking a question. Not the board's 5 and 900: the
         * verification sentence is appended in code and counts toward both
         * limits. The first two live hires were refused for exactly this, one
         * at 984 characters and one at six sentences.
         */
...
WRITE IT SO A STRANGER CAN EXECUTE IT. Two to FOUR sentences and 700 characters maximum: the check sentence is appended for you as the fifth, and the board refuses anything past five sentences or 900 characters. Title 3 to 80 characters. Say what the deliverable is, where it should end up, and what you will check before approving. Price it against what comparable jobs on the board actually paid, not against what you would like the work to be worth. Give a real deadline. Visible copy takes no hyphens or dashes, no gambling words, and "AI" rather than "A.I".
...
export function launchCommentPrompt(input: {
  ...
  return [
    ...
    `THE TEMPLATE YOU MUST NOT WRITE (operator audit 2026-09-18). Seven of the last twenty-four posts on this account were this one sentence with the nouns swapped, and it now reads as a bot filling in a form:\n  "<their line restated>. I launched <Name> on the Stonk Launcher for that <noun>, quoted in <TICKER>. <link>"\n  Real examples that shipped: "I launched Anthill on the Stonk Launcher for that density, quoted in STONK, contracts welcome." / "I launched Ex-Date Hound on the Stonk Launcher for that gap, quoted in AAPL." / "I launched Type Three Civilization on the Stonk Launcher for that destination, quoted in SPCX." Never write "for that <noun>". Never append "contracts welcome". Do not open every reply with their line restated, and do not make "I launched X on the Stonk Launcher" the second sentence every time. Vary what the reply is ABOUT: sometimes the mechanic the token encodes, sometimes the number behind it, sometimes what the lane choice means, sometimes a single dry observation with the link. If the only thing you can think of to say is the template, SKIP: a silent launch beats the eighth identical reply.`,
...
export function mintPrompt(
  ...
  return [
    ...
    `PAD OPTIONS (each verified by createLaunch simulation). The V2 pad IS the launcher's "Guaranteed Bond / anti snipe" family: your decaying start tax is the fair-start window and graduation is the bond, and the bonded LP mints into the Safety Deposit Box with no unlock and no admin key. Accepted knobs: eoaOnly true blocks contracts from buying; maxBuyPpm caps each wallet's buy in ppm of supply (10000 = 1%); bondVenue 1 graduates into Uniswap v3 instead of the StonkUp locker and is REQUIRED on arbweth, where bondVenue 0 reverts BadParam(); unsoldMode 1 is an alternate unsold-supply behaviour, keep 0 without a stated reason. Rejected today: openEnded false reverts BadParam() and validation refuses it. sellsEnabled false reverts BadEconomics(), but it is a REQUEST the executor probes at deploy time and falls back publicly when refused, so leave it true unless the concept is built on buy-only and the message is honest either way. Deviate as a disclosed choice, not decoration.`,
    `LENGTH BUDGET (hard): concept at most ${LAUNCH_TEXT_BUDGET} characters, rationale at most ${LAUNCH_TEXT_BUDGET}, message at most 400. A spec that runs long is truncated in review, never improved by it.

Design at most ONE launch spec, or return launch: null with a skipReason. Pad bounds: start mcap $1,000-$1,000,000; graduation $50,000-$10,000,000 and at least 2x start; start tax 0-9900 bps decaying by taxDecayPerMinuteBps a minute; buffer >= 600s.

LANE: take it from the LANE MENU and match it to the concept (GME lore on gme, AI and GPUs on nvda, rockets on spcx, consumer tech on aapl, oil and macro on uso, ecosystem plays on stonk, stable-quoted experiments on usdg, Arbitrum and cross-chain on arbweth, weth as the default when nothing fits better). Never pick a lane marked CLOSED. Do not reuse the lane of the last few launches unless the concept demands it. A stock-quoted lane makes the launch trade against the real stock token and accrues your fees in it, which is the strongest version of the meme-stock angle; your symbol must still be original.

RESERVED NAMES (hard rule): the floor and the Telegram bot HIDE any launch whose name or symbol contains ${RESERVED_LAUNCH_NEEDLES.map((n) => `"${n}"`).join(", ")} in any spelling ("Clock In", "CLOCK-IN", "cl0ckin" all count, and a needle split across name and symbol counts too). Such a launch deploys, pays the fee and is invisible everywhere. Code drops it before the queue.

EVERY LAUNCH CARRIES A MESSAGE. Launches are LAURA's public voice; the community watches each one appear. Fill the message field with the single statement this launch makes, make the name its headline and the concept its broadcast body. A launch with nothing to say is spam, so skip instead.

WHAT TO LAUNCH. Lead with meme-stock culture without being confined to it: symbols that read like tickers, GME/AMC-era energy, or a riff on stock-token news from the live intel. Never use a real company's actual ticker. Volume follows live narrative, not protocol statistics, so prefer a concept riding something in LIVE INTERNET INTEL today (the tokenized-stock tape, the meme-stock market list and boosted pitches, or the X pulse) and cite what you rode in the rationale. Lore, culture, event, community and deliberate economics experiments are all valid without a live current when the message is clear. Name the KIND in the rationale (live-current, lore, culture, event, community, experiment) and what you expect it to earn or teach.

CADENCE: no daily limit. Launch when there is something distinct to say, and pre-stage when the window is shut rather than skipping, since the executor paces deploys about 20 minutes apart. Never restate a message a recent launch made; a justified silence grades better than a repeated line.

VARIETY is the failure mode to beat. Across consecutive launches vary the kind, the lane, the scale ($1k experiments to $100k+ statements), the supply and the tax shape. NAMING (hard): never reuse a word from the NAME MATERIAL ALREADY SPENT list, rotate registers instead of stacking trading-floor jargon (internet culture, sports, space, food, mythology, machines, weather, small absurd objects), and use a number in a symbol only when the number IS the story.

LOGO: fill imageQuery with a concrete visual search phrase for the subject, not the ticker ("toll booth on a highway at night", not "toll economics"). Also pick artMotif (${ART_MOTIFS.join(", ")}), artPalette (${ART_PALETTES.join(", ")}) and artStyle (${ART_STYLES.join(", ")}) to match, and vary them across launches. If ${queueLimit}+ LAURA launches are already pending, skip.`,
    `TAX DESIGN. 16.5% of every taxed trade is push-paid to LAURA in the lane's quote token the moment it lands, so on the stonk lane the income arrives as $STONKBROKER while the trade prints mission-token volume. Ranges: startTaxBps 0-9900 decaying by taxDecayPerMinuteBps a minute; postTaxBps 100-500, and the pad REVERTS below 100. DECAY RULE (the pad reverts BadEconomics() otherwise): startTaxBps must be an EXACT MULTIPLE of taxDecayPerMinuteBps and the window startTaxBps/taxDecayPerMinuteBps must be 10 to 99 minutes. 2000/200, 2500/125 and 3000/50 are valid; 2000/250, 2000/400 and 1000/99 revert. Pick the window first, then divide. Design the tax as part of the message and say what it is for in the token's own words: an unexplained heavy tax reads as a rug signal. No return promises, and LAURA never buys her own tokens.`,
    `FEE LIFECYCLE. Curve phase: 16.5% of each trade's tax arrives instantly in the lane's quote token. After graduation the pool is locked forever but LAURA keeps the lock NFT and collects 80% of that pool's swap fees for as long as it trades, so a graduated token is a permanent revenue stream, not a finished one. Claiming is automatic; never spend words asking for it. Three consequences for your design: concepts that give people a reason to keep trading out-earn one-shot jokes, the lane decides which asset the treasury accrues, and an achievable gradMcapUsd converts a temporary tax stream into a permanent fee stream.`,
  ].join("\n\n");
}

/**
 * Ticker's launch brief (operator directive 2026-09-13: "allow ticker to
 * deploy launchpad tokens with sells disabled"). Ticker reads the token tape
 * for a living, so its launches ride what the market is actually doing right
 * now, and it owns the buy-only design space: sellsEnabled false is a request
 * the executor probes against the pad, falling back publicly when refused.
 */
export function tickerLaunchPrompt(
  ctx: CycleContext,
  tape: string,
  pendingLaunches: number,
  spokenDigest: string,
  ...
  return [
    ...
    `WHAT LAURA HAS ALREADY SAID (recent launches; never repeat a statement or reuse name material)\n${spokenDigest}`,
    `YOUR SKILLS (operating procedures; follow them)\n${ctx.skills.tokenintel ?? "None."}`,
    `LIBRARY (durable build knowledge; the launch playbook and verified pad rules live here)\n${ctx.library.slice(0, 9000)}`,
    `YOU ARE TICKER, AND THIS IS YOUR LAUNCH TURN (at most once every few hours; Mint launches on the cycle pace, you launch when the tape says something). Design ONE launch spec for a Smart Launch V2 pad that turns a live market fact into a token people watching the launcher will understand instantly: the strongest mover on the tape, a meme-stock current the tokenized-stock lanes are riding, a liquidity or volume shift, a launcher token that just graduated. The concept and message must name the market fact with its number and date. Return launch: null with a skipReason when the tape is quiet or the queue is full; a skip is a real verdict.`,
    `BUY-ONLY CURVES (operator directive 2026-09-13): you may set sellsEnabled false to request a buy-only curve, a token whose curve phase accepts buys only until graduation. Know the facts: the V2 pads refuse buy-only today (createLaunch reverts BadEconomics(); re-verified by simulation on 2026-09-13 in every parameter combination). The executor probes the pad the moment your launch deploys: if the pad accepts, the curve is buy-only as designed; if it refuses, the SAME launch deploys with sells enabled and the fallback is written on the launch record and in the event log. So request buy-only only when the concept is built around it, and write the message so it is honest in both outcomes (for example, frame it as "a one-way curve if the pad allows it, an open curve if not"). Never promise buy-only.`,
    `HARD RULES you share with Mint: names and symbols original and non-deceptive, never a live token's name, never a person's or a project's name, never the operator's; pad bounds from the LANE MENU (start mcap $1k-$1M, graduation $50k-$10M and at least 2x start, buffer >= 600s, postTaxBps 100-500, start tax an exact multiple of the decay with a 10-99 minute window); pick artMotif, artPalette, artStyle and imageQuery that fit the concept. NO SIGN-OFF: the concept and message end when the thought ends; no disclaimers, no disclosures about what LAURA is. LENGTH BUDGET: concept at most ${LAUNCH_TEXT_BUDGET} characters, rationale at most ${LAUNCH_TEXT_BUDGET}, message at most 400.`,
  ].join("\n\n");
}
