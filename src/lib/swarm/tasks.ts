...
export function mintPrompt(
  ...
  return [
    ...
    `ADVANCED PAD OPTIONS (operator directive 2026-09-11 — the full customization surface is yours; every claim below was verified by createLaunch simulation on the live pads). First, know what you already have: the V2 Safe Launch pad IS the launcher's "Guaranteed Bond / anti snipe" launch family — official copy: "snipe tax up to 99% falling every minute, raise bonds at the bell", and at graduation "the LP position mints into the Safety Deposit Box, a permanent locker with no unlock and no admin key". Your anti-snipe start tax and the guaranteed bond are the same design: the decaying tax IS the fair-start window, and graduation IS the bond. New knobs verified ACCEPTED on-chain: eoaOnly true = anti-bot shield (contracts cannot buy); maxBuyPpm = per-wallet anti-snipe whale cap in ppm of supply (10000 = 1% per wallet; combine with the decaying tax for fair-start designs the message narrates); bondVenue 1 = graduate into the Uniswap V3 venue instead of the StonkUp CL locker (LP still locks, fee rights stay with LAURA) — REQUIRED on arbweth / Arbitrum One, where StonkUp does not exist and bondVenue 0 reverts BadParam() (verified 2026-09-15); unsoldMode 1 = alternate unsold-supply behavior at bond (keep 0 without a stated reason). Modes the pads REJECT today: openEnded false (closed window) reverts BadParam() on every pad in every combination and spec validation refuses it. sellsEnabled false (buy-only) also reverts BadEconomics() today, but since 2026-09-13 it is a REQUEST the executor probes at deploy time, falling back to sells enabled publicly when the pad refuses; Ticker owns buy-only concepts, so leave sellsEnabled true unless a concept genuinely needs the request and the message is honest in both outcomes. Defaults remain right for most launches; deviate as a DESIGNED, DISCLOSED choice, not decoration. TRY ALL LANES over time: weth, stonk, usdg and each open stock lane should all see launches; the menu shows what you have neglected.`,
